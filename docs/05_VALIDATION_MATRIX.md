...
| Driver unit tests (ES8311 codec) | **Verified** | `tests/drivers/audio/es8311` (native_sim) | I2C emulator + ztest on native_sim, 11/11 pass: chip-ID (+ wrong-ID warn), 16 kHz/16-bit playback + capture (ADC) configure + write order, volume/mute, unsupported route/format/property rejection, I2C-error propagation. Logic-verified |
...
| ES8311 audio capture / mic (HW-016) | **Partial — transport runtime-verified, analog mic NOT verified** | `evidence/20260605-hw016-audio-{analog-mic,digital-loopback,capture,build,flash}.*` | Issue #6. A `0x44 = 0x68` digital-mux loopback reads a bit-stable rms=4089/peak=5800: the ASDOUT serial out + I2S0 full-duplex RX + capture DSP are runtime-verified end to end. The shipping real-ADC route (`0x44 = 0x08`) reads rms=0 (peak <= 8) on BOTH slots during the beep — the ADC capture route is silent. The 0x68 path bypasses the ADC modulator, so the fault is NOT localized (analog front end / ADC modulator-power-clock / the 0x08 mux routing) and is NOT separated from an unconfirmed acoustic stimulus. #6 acceptance NOT met; #6 stays OPEN, analog-ADC bring-up continues in a focused follow-up. native_sim: es8311 11/11 + audio_dsp. LCD photo `evidence/PXL_20260604_190722170.MP.jpg` shows the AUDIO page `mic bar=0 rms` on hardware. INTERIM (transport-only) milestone |

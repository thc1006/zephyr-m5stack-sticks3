...
| IR TX/RX (NEC) | Roadmap (in progress) | see HW-005 | Zephyr 4.4 has no ESP32 RMT driver; TX via LEDC carrier (G46), RX via MCPWM capture (G42); NEC encode/decode unit-tested. Gated `CONFIG_APP_IR`. Not HW-confirmed until the TX→RX loopback evidence is captured |

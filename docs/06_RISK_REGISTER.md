...
| **Zephyr's ESP32 I2S driver leaks a mem-slab block on every START/DROP, and the app then HANGS** | High | **Certain (found on hardware, HW-019)** | `drivers/i2s/i2s_esp32.c`: `i2s_esp32_{rx,tx}_stop_transfer()` set `stream->data->mem_block` - the block the DMA is working on - to NULL **without returning it to the slab**, and `I2S_TRIGGER_DROP` calls exactly those. The queue purge that follows only frees what is in the message queue. So one block per direction is lost per cycle. It does not surface as an error: `i2s_buf_write()` allocates with `K_FOREVER` (`drivers/i2s/i2s_common.c`), so an exhausted slab is an **unkillable block with no log line**. Measured on silicon with `BLOCK_COUNT = 8`: free counts fell 8,7,6,5,4,3,2,1 across eight reconfigures and the ninth hung. **This hits our own audio app**: every capture session ends in a DROP, so opening the AUDIO page eight times and then a ninth would hang the device. Present in v4.4.0 **and on current main**. MITIGATION: `scripts/patch_zephyr_i2s_leak.sh` (three hunks; the TX callback must NULL `mem_block` after freeing it first, or the stop-path free becomes a double free). Hardware-proved: with it the slab census reads 8/8 at every rate and the sweep completes. `audio_slab_ok()` refuses to start a stream on an exhausted slab and names the fix, so an unpatched build gets a message instead of a mystery. To be reported upstream |
